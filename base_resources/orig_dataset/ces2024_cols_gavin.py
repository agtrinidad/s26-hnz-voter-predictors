CES2024_COLUMNS = {
    ...
    # ── Demographics ── Each one is a practical variable for targeting
    "birthyr": "Birth Year",
    "gender4": "Gender",
    ...
    # ── Multiracial (check all) ── use, can identify demographic groups
    ...
    "votereg": "Voter Registration Status", ## YES!
    "pid3": "3 point party ID", # Yes! assuming D vs I vs R.
    "pid3_t": "3 point party ID (text)",
    "pid7": "7 point Party ID",

    # ── Geography ──
    "inputstate": "State of Residence", #Yes -
    "region": "Region", # Yes
    "ccesmodule": "Team module assigned",

    # ── Media use past 24 hrs (check all) ── interesting for practical outreach predictors?
    ...
    # ── Media use networks (check all) ── Yes. Perhaps FE for liberal, centrist, republican news to reduce dimensionality
    ...
    # ── Life Changes (check all) ── Just doesn't seem predictive OR actionable
    ...
    # ── Ukraine (check all) ── FE for different levels of involvement?
    ...
    # ── Israel/Gaza (check all) ── FE for different levels of involvement?
    ...
    # ── Emergency expense (check all) ── Income proxy? Perhaps redundant
    ...
    # ── Know Party in Government (grid) ── Yes!  voter knowledge
    ...
    # ── Know Party of Representative (grid) ── Yes ! voter knowledge
    ...
    # ── Job Approval Institutions (grid) ── Seems reasonable for voter turnout. IE, if approve X in this election, more likely to vote
    # Drawback - backward looking approval and party flip means harder to parse out generalizablity
    ...
    # ── Citizenship & Immigration ── Immigration background may be interestng to include. US citizen should be a filter for data
    "cit1": "US citizen",
    "immstat": "Immigration background",

    # ── Gun control (grid) ── FE to reduce dimensionality?
    ...
    # ── Immigration policy (grid) ── FE to 2. Pro vs con immigration
    ...
    # ── Abortion (grid) ── Granularity is harder to FE. Maybe remove or keep all?
    ...
    # ── Environment (grid) ── FE to pro vs con CC efforts
    ...
    # ── Tax/spending policies (grid) ── very granular and harder to FE.
    ...
    # ── Congress Bills (grid) ── outdated bills, not exceptionally interesting
    ...
    # ── Tax rates (grid) ── would definitely be seperate predictors. Not actionable tho. 
    ...
    # ── Party registration & past vote ── YES! Useful info and actionable.
    ...
    # ── Pre-election vote intention & preferences ── some of this seems like noise but first two should be grabbed
    ...
    # ── Residency & community ── YES! great for targeting and probably predictive
    "CC24_368": "Residency",
    "urbancity": "Type of Area Living In",
    "urbancity_t": "Type of Area Living In (text)",
    "ideo5": "Ideology",

    # ── Parenting ── Worth throwing in. Restricts ability to spend free time voting. Not convinced it's a strong predictor, but useful to include for now
    ...
    # ── Employment ── Yes, another economic predictor
    "employ": "Employment Status",
    "employ_t": "Employment Status (text)",
    "hadjob": "Had job in past 5 years",
    "investor": "Stock ownership",

    # ── Religion ── curious. Seems interestingly actionable. IE church/temple/mosque drives to vote/register.
    ...
    # ── Marital & Union ── Union may be interesting. Non-partisan institution may struggle to use
    ...
    # ── Military Household (check all) ── include. pretty big accesibility to vote issue here
    ...
    # ── Health insurance (check all) ── seems like noise. Id drop in favor of more distinct income or health policy questions
    ...
    # ── Middle Eastern or North African ── Seems too granular. Lean towards including top-level identifier
    ...
    # ── Latin heritage (check all) ── Lean towards including top-level identifier. May need to FE this
    ...
    # ── Asian heritage (check all) ── FE to make an asian category?
    ...
    # ── Native American ── Value in reservation outreach?
    "CC24_native": "Native American heritage",

    # ── Past presidential vote ── not a fan. Dont see the utility
    ...
    # ── District/Geography (pre-election) ── Include. geographic targeting is cool
    ...
    # ── Contextual: current officeholders & candidates (pre-election) ── DELETE AND DROP. not actionable or predictive at the person level
    ...
    # ── Page timing (pre-election) ── Filter, but dont like the idea of using as predictor
    ...
    # ── Post-election: Party ID ── -- interesting if we can FE a swing voter category.
    ...
    # ── Turnout ──
    "CC24_401": "Voted in 2024 General election", ### USE!
    "CC24_402a": "Main reason didn't vote",  ### explore, but probably use?
    ...
    # ── Vote method ── include how they voted vars. Time indicators may be good too. Need a convincing institutional action story tho to use predictors in recomm
    ...
    # ── Voting problems ── include. Definitely impacts avility to vote
    ...
    # ── Post-election vote choice ── Not a fan
    ...
    # ── Post-election: non-voter preferences ── Not a fan
    ...
    # ── Use U.S. military troops (check all) ── not a fan
    ...
    # ── Election fairness (grid) ── Curious. Id throw this in to see how it aides the model
    ...
    # ── Trust in government ── Definitely include. If predictor, then question about building trust in institutions --> turnout
    "CC24_423": "Trust in federal government",
    "CC24_424": "Trust in state government",

    # ── Past year political activities (check all) ── Include. maybe FE a political activity variable
    ...
    # ── Donate money (check all) ── FE a political money category. Ordinal?
    ...
    # ── Campaign contact ── Include? Touch pount indicator
    ...
    # ── Run for office ── Include. Probably dimension reduce
    ...
    # ── Racial/Sexual agreement (grid) ── Unclear utility? 
    ...
    # ── Racial Resentment (grid) ── Unclear utility.
    ...
    # ── State legislature spending (grid) ── Drop
    ...
    # ── State policies (grid) ── Seems redundant with other abortion questions. Noise
    ...
    # ── Supreme Court decisions (grid) ── Maybe throw in first pass. Not a fan.
    ...
    "numchildren": "Number of children", #redundant?
    "gigwork": "Gig work", ## YES YES
    "edloan": "Student debtor", #  curious, not amazing but would like to include
    "student": "Student Full or Part-time", ### YES YES YES

    # ── Post-election geography & contextual ── DROP
    ...
    # ── Page timing (post-election) ── DROP, but maybe filter
    ...
    # ── Vote Validation (TargetSmart) ── curious. Expore more.
